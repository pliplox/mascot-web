...
export const AuthProvider = ({ children }) => {
  ...
  const signIn = async (email, password) => {
    try {
      ...
      if (response.status >= 400) {
        setAuthError(response.data.err || response.data.message); // FIXME: err -> message
      } else {
  ...
  const signUp = async (name, email, password) => {
    try {
      ...
      if (response.status >= 400) setAuthError(response.data.err || response.data.message); // FIXME: err -> message
      return response;
    } catch (error) {
      console.log(error);
      return setAuthError(error.message);
